...
import { MessageSquare, History, Loader2, Copy, Trophy, Handshake as HandshakeIcon } from 'lucide-react';
import type { Game as GameType, Player, PlayerColor, PlayerProfile } from '@shared/types';
...
export function GamePage() {
  ...
  const handleGameOver = useCallback((status: ReturnType<typeof getGameStatus>, currentPlayers: [Player, Player | null]) => {
    if (status.status !== 'ongoing' && status.status !== 'waiting') {
      ...
      switch (status.status) {
        ...
        case 'stalemate': message = 'Stalemate! The game is a draw.'; break;
        default: message = 'Game over. The result is a draw.';
      }
  ...
  }, [gameMode, gameId, isAuthenticated, fetchGame, joinGame, navigate, fetchSelfProfile]);
  useEffect(() => {
    if (gameMode !== 'online' || gameOver) return;
    const interval = setInterval(fetchGame, 2500);
    return () => clearInterval(interval);
  }, [gameMode, gameOver, fetchGame]);
  const onMove = useCallback((sourceSquare: Square, targetSquare: Square, piece: ChessboardPiece): boolean => {
    ...
    if (gameMode === 'online') {
      ...
      // Send move to backend in the background
      api<GameType>(`/api/games/${gameId}/move`, {
        method: 'POST',
        body: JSON.stringify({ playerId, move: moveData }),
      }).catch(error => {
        toast.error(error.message || "Invalid move");
        // Revert on API error by fetching latest state
        fetchGame();
      });
  ...
  }, [fen, game, gameMode, gameData, gameId, playerId, playCapture, playMove, playCheck, handleGameOver, fetchGame]);
  const handleOfferDraw = async () => {
    if (gameMode !== 'online' || !gameId || !playerId) return;
    try {
      const updatedGame = await api<GameType>(`/api/games/${gameId}/offer-draw`, { method: 'POST', body: JSON.stringify({ playerId }) });
      updateGameState(updatedGame);
      toast.info("You have offered a draw.");
    } catch (error: any) { toast.error(error.message || "Failed to offer draw."); }
  };
  const handleRespondToDraw = useCallback(async (accept: boolean) => {
    if (gameMode !== 'online' || !gameId || !playerId) return;
    try {
      const updatedGame = await api<GameType>(`/api/games/${gameId}/respond-draw`, { method: 'POST', body: JSON.stringify({ playerId, accept }) });
      updateGameState(updatedGame);
      toast.dismiss('draw-offer');
      if (accept) toast.success("Draw accepted. The game is over.");
      else toast.warning("Draw offer declined.");
    } catch (error: any) { toast.error(error.message || "Failed to respond to draw offer."); }
  ...
  const whitePlayer: Player | null = gameData?.players[0] ?? (gameMode !== 'online' ? { id: 'p1', name: gameMode === 'ai' ? 'You' : 'PLAYER1 (White)' } : null);
  const blackPlayer: Player | null = gameData?.players[1] ?? (gameMode !== 'online' ? { id: 'p2', name: gameMode === 'ai' ? 'AI Opponent' : 'PLAYER2 (Black)' } : null);
  const opponentPlayer = playerColor === 'w' ? blackPlayer : whitePlayer;
    ...
    <div className="container mx-auto p-4">
      ...
      <div className="flex flex-col lg:flex-row gap-8 items-start justify-center">
        <div className="w-full flex-1 flex flex-col items-center gap-4">
          <PlayerInfo player={blackPlayer} selfProfile={selfProfile} isOpponent={blackPlayer?.id === opponentPlayer?.id} isBlack={true} onFriendAction={fetchSelfProfile} />
          ...
          <PlayerInfo player={whitePlayer} selfProfile={selfProfile} isOpponent={whitePlayer?.id === opponentPlayer?.id} isBlack={false} onFriendAction={fetchSelfProfile} />
        </div>
